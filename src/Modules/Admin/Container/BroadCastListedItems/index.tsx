...
function BroadCastListedItems ({ item }: BroadCastItemsProps) {
    const {title,attachments, description, created_by, created_at,company } = item
    ...
    <div className="row d-flex justify-content-center pt-2 ">
        ...
        <div className="row mx-sm-0 mx--5">
          <div className="col pl-sm-0 pl-4 pl-lg-3">
           <H tag={"h4"} className="text-capitalize font-weight-900 " text={title} />
          </div>
        ...
        <div className="row mx-sm-0 mx--5">
          ...
          company?.map((company: any, index: number) => {
            return (
             <H tag={"h3"} className="text-capitalize font-weight-900 ls-15 mb--1 pl-sm-0 pl-2 " text={company.register_name} />
            );
          })}
            </div>
         
            <div className='col-auto pt-2 pl-sm-0 pl-4 pl-lg-1'>
                <div className='text-xs font-weight-600'>
                    created by
                
                </div>
                <H tag={"h5"} className="text-capitalize font-weight-900 mt--1 " text={created_by.name} />
        ...
        {attachments &&
          ...
          attachments?.map((attachment_logo: any, index: number) => {
            return (
                <Image variant={'default'}  src={getPhoto(attachment_logo.attachment_file)} width={'100px'} height={'100px'} />
            );
          })}
