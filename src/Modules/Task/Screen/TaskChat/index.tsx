        ...
        <>
                ...
                item.tagged_users?.length > 0 && item.tagged_users.map((taggedElements) => {
                    ...
                    return (
                        <div className={'d-flex justify-content-start text-lightGray mb-1'}>
                            <div className={'ml-lg-4 mr-md-4'}>{`@${taggedElements.name} tagged by ${item.by_user.name}`} </div>
                        </div>
                    )
                })
            }

            {
                item?.assigned_to?.name === undefined ? null :
                    <div className='d-flex justify-content-start text-lightGray mb-1'>
                        <div className={'ml-lg-4 mr-md-4'}>{`@${item?.assigned_to?.name} tagged by ${item.by_user.name}`}</div>
                    </div>
            }
